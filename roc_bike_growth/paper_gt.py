...
def gt_with_existing_full(G_base, G_existing, route_factor=0, prune_factor=1, prune_measure = "betweenness", by_factor ="mod", distance_limit = 99999999999):
    ...
    if(prune_measure == 'hybrid'):
        G_post = ig.Graph.from_networkx(G_nx)
        
        for e in G_post.es:
            if(e['length'] == None):
                G_post.delete_edges(e)
        ...
        G_gen = iterative_pruning(G_post ,G_existing, prune_factor, by_factor)
        G_nx = gu.ig_to_nx(G_gen)
...
def iterative_pruning(GT, existing, prune_factor, by_factor):
    ...
    if(by_factor == "mod"):
        ...
        '''elif(type(by_factor) == list):
        if(len(by_factor > 6):
           by_factors = by_factor[0:5]'''
    else:
